...
pub struct LLMDirector {
    pub in_flight_prompts:HashSet<usize>,
    pub latest_id:usize,
...
impl LLMDirector {
    pub fn new_with_goals(goals:Vec<String>) -> Self {
        Self { in_flight_prompts: HashSet::new(), latest_id: 0, responses: Vec::new(), goals, last_prompt_tick: 0 }
    }
    pub fn get_periodic_payload<'a>(
        ...
        if tick - self.last_prompt_tick > 1000 && self.in_flight_prompts.len() == 0 {
            Some(self.get_payload(agent_id, DirectorAlert::Periodic, first_ent, second_ent, world, tick))
        }
        else {
            None
        }
    ...
    pub fn get_payload<'a>(
        ...
        HordeProximaAIRequest::new(
            ...
            EndpointRequestVariant::RespondToFullPrompt { 
                ...
                vec![
                    ChatSetting::ResponseTokenLimit(500),
                    ChatSetting::Temperature(70)
                ])) 
            }
        )
    ...
    fn parse_response<'a>(
        ...
        self.in_flight_prompts.remove(&response.request_id);
                ...
                match words[0] {
                    ...
                    "GOTO" => if words.len() == 4 {
                        if let Ok(x) = words[1].parse::<i32>() && let Ok(y) = words[2].parse::<i32>() && let Ok(z) = words[3].parse::<i32>() {
                            ...
                            first_ent.tunnels.actions_out.send(ActionsEvent::new(agent_id, None, ActionsUpdate::AddAction(Action::new(id, tick, ActionTimer::Delay(500), ActionKind::PathToPosition(Vec3D::new(final_position.x as f32, final_position.y as f32, final_position.z as f32), 0.8), ActionSource::Director))));
                        }
                    },
                    "GOAL" => match words[1] {
                        "add" => {
                        ...
                        "remove" => if let Ok(number) = words[2].parse::<usize>() {
                            match number.checked_add_signed(-1) {
                                Some(real_number) => if self.goals.len() > real_number {
